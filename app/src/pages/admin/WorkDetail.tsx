        ...
        {admin && (
          <TabsContent value="finance" className="space-y-6">
            ...
            {(() => {
              ...
              // fatDireto já inclui o transformer das medições anteriores
              ...
              const saldoContrato = base - fatDireto; // base de serviços (contrato - fat. direto)
              const saldoAMedirRaw = saldoContrato - medidoTotal;
              // Arredondar e tratar diferença de centavo como zero
              ...
              return (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
                    ...
                    <Card className={`border-l-4 ${saldoAMedir > 0 ? 'border-l-teal-500' : saldoAMedir === 0 ? 'border-l-emerald-500' : 'border-l-red-600'}`}>
                      <CardContent className="p-3">
                        <p className="text-[10px] text-slate-500 uppercase font-medium">
                          {saldoAMedir === 0 ? '✅ A Medir' : '📏 A Medir'}
                        </p>
                        <p className={`text-xl font-bold font-mono ${saldoAMedir > 0 ? 'text-teal-600' : saldoAMedir === 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                          R$ {fmt(Math.abs(saldoAMedir))}
                        </p>
                        <p className={`text-[9px] mt-0.5 ${saldoAMedir === 0 ? 'text-emerald-600 font-semibold' : 'text-slate-400'}`}>
                          {saldoAMedir === 0 ? 'Totalmente medido' : `Medido: R$ ${fmt(medidoTotal)}`}
                        </p>
                      </CardContent>
                    </Card>
                  </div>
