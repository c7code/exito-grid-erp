...
export default function DailyLogTab({ dailyLogs, rentals, equipment: _equipment, employees, reload }: Props) {
  ...
  async function save() {
    ...
    try {
      await api.createEquipmentDailyLog({
        ...form,
    ...
    } catch { toast.error('Erro ao registrar diária'); }
  }
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setDlgOpen(true)} className="bg-cyan-600 hover:bg-cyan-700 text-white font-semibold">
            ...
            return (
              <Card key={rentalId} className="overflow-hidden">
                ...
                <div className="divide-y">
                  {(logs as any[]).map(log => (
                    <div key={log.id} className="flex items-center gap-4 px-5 py-3 hover:bg-slate-50/30">
      ...
      <Dialog open={dlgOpen} onOpenChange={setDlgOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Registrar Diária</DialogTitle></DialogHeader>
          ...
          <DialogFooter className="mt-4">
            ...
            <Button onClick={save} className="bg-cyan-600 hover:bg-cyan-700 text-white">Registrar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
